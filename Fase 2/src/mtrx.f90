module matrix_m
  ...
  subroutine recorrerMatriz(self, mtx)
    class(matrix_t), intent(inout) :: self
    type(matrix_t), intent(inout) :: mtx
    type(header_node), pointer :: row_hdr_node
    type(header_t), pointer :: row_hdr, col_hdr
    type(matrix_node), pointer :: current_node
    integer :: row, col
    integer :: num_rows, num_cols
    ...
    do while(associated(row_hdr_node))
      ! Iniciar en el primer nodo de la fila actual
      current_node => row_hdr_node%access
      do while(associated(current_node))
       
            call mtx%add(current_node%row, current_node%col, current_node%value)
  ...
  subroutine create_dot(self, filename)
    ...
    !code = code // ' splines=false;' // new_line('a')
  ...
  function get_content(self) result(content)
    ...
    do while (associated(row_node))
      ...
      do while (associated(current_mtx_node))
        ...
        createMtxNodes = createMtxNodes // '" group = ' // trim(aux2) // ', style = filled, fillcolor = "'&
        //trim(current_mtx_node%value)//'"];' // new_line('a')
        ! align matrix nodes
  ...
  subroutine write_dot(self, code, filename)
    ...
    close(10)
    call system("dot -Gnslimit=2 -Tpng graph.dot -o "//&
    trim(filename)//"grafo.png")
